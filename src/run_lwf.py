...
from models import LearningWithoutForgetting
...
if __name__ == '__main__':
    ...
    PARSER.add_argument('--model_type', '-md', type=str, help='Select the model',
                        choices=['LWF', 'LWF_E'], default='LWF')
    ...
    if CONFIG.init_model != '':
        MODEL = LearningWithoutForgetting(model_type=CONFIG.model_type, n_epochs=CONFIG.n_epochs, device=CONFIG.device,
    ...
    for SESSION, SESSION_DATA in enumerate(DATA_SESSIONS):
        ...
        if CONFIG.model_type == 'LWF_E':  # Utilize the exemplars
